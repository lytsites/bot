...
def build_greeting_prompt(system_prompt: str, peer_username: str | None, peer_display_name: str | None) -> str:
    ...
    header = (
        ...
        "- IMPORTANT: Generate UNIQUE greetings each time. Avoid repeating the exact same phrase.\n"
        "- Vary your vocabulary: use synonyms, different sentence structures, and alternative expressions.\n"
        "- Examples of varied greetings:\n"
        "  * 'Салам, братан, есть чё?'\n"
        "  * 'Что по товару?'\n"
        "  * 'Прайс скинь'\n"
        "  * 'Брат, что по весу?'\n"
        "  * 'Бро, чё по товару?'\n"
        ...
        "  * 'Добрый день, что по товару?'\n"
        "  * 'Приветствую, прайс скинь'\n"
        "  * 'Здравствуй, что по весу?'\n"
        "  * 'Добрый, есть чё?'\n"
        "  * 'Привет, что есть?'\n"
        "  * 'Здорово, брат, что по товару?'\n"
        "  * 'Есть чё?'\n"
        "  * 'Быстро, что по товару?'\n"
        "  * 'Быстро, брат, прайс скинь'\n"
        "  * 'Чё там по товару?'\n"
        "  * 'Прайс'\n"
        "  * 'Что есть?'\n"
        "  * 'Что по весу?'\n"
